...
# 1. THE NEW TOOLS (Added SocketIO, NO eventlet to protect asyncio)
from flask_socketio import SocketIO

app = Flask(__name__)
CORS(app) # Allows your laptop to talk to the PC without CORS blocks
...
async def listen():
    ...
    global latest_stats # Added for the Data Office
    
    uri = "wss://stream.binance.com:9443/stream?streams=btcusdt@aggTrade/btcusdt@depth20"
    
    # --- THE IMMORTAL ENGINE LOOP ---
    while True:
        try:
            # Added ping_interval and ping_timeout to keep the connection alive
            async with websockets.connect(uri, ping_interval=20, ping_timeout=10) as websocket:
                ...
                #---Thermodynamic "Free Energy" ($G = U - TS$)---
                    ...
                    # UPDATE THE GLOBAL "DATA OFFICE" VARIABLE
                    latest_stats = payload
                    
                    # 3. THE SCREAMER (Pushes data to laptop instantly)
                    ...
                    # --- HTML DASHBOARD (Updated to use Socket.IO) ---
                    html_content = r"""
                    <!doctype html>
                    <html lang="en">
                    <head>
                    ...
                    // --- NEW PUSH LISTENER (Replaces the slow fetch interval) ---
                    const socket = io('http://192.168.225.224:5000');
...
# --- 4. THE "IGNITION" (Updated to use Socket.IO) ---
if __name__ == "__main__":
    # Start SocketIO server in the background so it doesn't block asyncio
    Thread(target=lambda: socketio.run(app, host='0.0.0.0', port=5000, debug=False, use_reloader=False)).start()
    
    # Start your original Trading Engine/WebSocket loop here
    asyncio.run(listen())
